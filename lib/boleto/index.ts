...
// TEM O BOLETO GENERATOR DO COBRANÇA. SEMPRE QUE MEXER AQUI, TEM QUE MEXER IGUAL LÁ PARA MANTER A FUNCIONALIDADE.
// ISSO É MUITO IMPORTANTE!!!!!!!!!!!!!
//* SEMPRE QUE FOR MEXER LÁ, TEM QUE MEXER AQUI(SE FOR MUDAR A FUNCIONALIDADE)
// Usamos require para forçar caminho CommonJS do gerar-boletos/pdfkit/fontkit e evitar o módulo ESM que quebra com swc.
...
class LegacyItau extends ItauBanco {
  ...
  geraCodigoDeBarrasPara(boleto: any) {
    ...
    return new (require("gerar-boletos/lib/generators/barcode-builder"))(boleto).comCampoLivre(campoLivre)
  }
}
...
export async function generateItauBoletoPdf({
  ...
  const boleto = {
    ...
    boleto: {
      // Formato legado: 7 dígitos + DV mod10
      numeroDocumento,
      especieDocumento: "DM",
    ...
    instrucoes: [
      // Instruções/mensagens do legado
...
// Igual ao legado: CalcularDVSantander
...
// Usa o mesmo padrão da biblioteca gerar-boletos, com DV calculado via mod11Santander
class LegacySantander extends SantanderBanco {
  // Sobrescreve para usar nosso cálculo de DV do nosso número (mod11Santander)
  // A biblioteca original espera que o DV seja passado via beneficiario.getDigitoNossoNumero()
  // Aqui garantimos que o campo livre seja montado corretamente
  ...
  geraCodigoDeBarrasPara(boleto: any) {
    ...
    // Usa o builder padrão da biblioteca que monta código de barras + linha digitável
    return new (require("gerar-boletos/lib/generators/barcode-builder"))(boleto).comCampoLivre(campoLivre)
  }
...
export async function generateSantanderBoletoPdf({
  ...
  // (ver boletoStringify.js linha 32: .comCodigoBeneficiario(dadosBancarios.conta))
  // Para Santander, o código do beneficiário é diferente da conta corrente
  ...
  const boleto = {
    ...
    beneficiario: {
      ...
      dadosBancarios: {
        ...
        // Então passamos o código do beneficiário no campo "conta"
