...
export const summarizeNode = async (state: MultiAgentState) => {
    ...
    const prompt = ChatPromptTemplate.fromTemplate(
        `Você é um assistente de resumo de conversas de vendas de lustres (inbound).
Resuma a conversa até agora, mantendo detalhes essenciais:
- Intenção de compra (modelos de interesse).
- Dúvidas técnicas ou estéticas (ambientes, tipos de luz).
- Se já houve consulta ao estoque.
- Se o cliente está pronto para fechar a compra.
...
Gere o novo resumo consolidado em português, de forma concisa (máximo 100 palavras):`
    ...
    console.log(`[📝 Lustres Summary Updated] -> "${response.substring(0, 100)}..."`);
