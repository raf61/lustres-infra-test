...
import { getSendMessageQueue } from '../infra/queue/send-message.queue';
...
export class SendMessageUseCase {
  ...
  async execute(input: SendMessageInput): Promise<SendMessageResult> {
    ...
    // 7. Enfileirar para envio (igual ao Chatwoot SendReplyJob)
    ...
    console.log(`[SendMessageUseCase] Message ${message.id} (${messageType}) created and queued for sending`);
